			...
			struct list_head *uf)
{
	...
	*populate = 0;
	...
	if (flags & MAP_NAMED_SWAP) {
		...
		/* Peek at our future neighbors based on the finalized addr */
		next = find_vma_prev(mm, addr, &prev);

		/* Check if we can adopt the LEFT neighbor's file */
		if (prev && prev->vm_end == addr && vma_is_named_swap(prev)) {
			file = prev->vm_file;
			/* Synchronize pgoff so is_mergeable_vma allows the merge! */
			pgoff = prev->vm_pgoff + vma_pages(prev);
		} 
	...
	if (file) {
		...
		case MAP_PRIVATE:
			...
			if (!file->f_op->mmap)
				return -ENODEV;
			if (vm_flags & (VM_GROWSDOWN|VM_GROWSUP) && !mapping_named_swap(file->f_mapping))
				return -EINVAL;
			break;
